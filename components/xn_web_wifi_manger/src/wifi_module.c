...
esp_err_t wifi_module_init(const wifi_module_config_t *config)
{
    ...
    if (s_wifi_cfg.enable_ap) {
        ...
        /* 设置 AP MAC 地址（BSSID） */
        if (s_wifi_cfg.ap_bss_id[0] != '\0') {
            uint8_t mac[6] = {0};
            int matched = sscanf(s_wifi_cfg.ap_bss_id, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                                &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
            
            if (matched == 6) {
                ret = esp_wifi_set_mac(WIFI_IF_AP, mac);
                if (ret != ESP_OK) {
                    ESP_LOGW(TAG, "esp_wifi_set_mac(AP) failed: %s, using default MAC", 
                             esp_err_to_name(ret));
                } else {
                    ESP_LOGI(TAG, "AP MAC set to: %s", s_wifi_cfg.ap_bss_id);
                }
            } else {
                ESP_LOGW(TAG, "Invalid AP BSSID format: %s, using default MAC", 
                         s_wifi_cfg.ap_bss_id);
            }
        }
    }
